/* This file was generated by SableCC (http://www.sablecc.org/). */

package org.openflexo.foundation.fml.parser.node;

import org.openflexo.foundation.fml.parser.analysis.Analysis;

@SuppressWarnings("nls")
public final class AIdentifierPrefix extends PIdentifierPrefix
{
    private TLidentifier _lidentifier_;
    private TDot _dot_;

    public AIdentifierPrefix()
    {
        // Constructor
    }

    public AIdentifierPrefix(
        @SuppressWarnings("hiding") TLidentifier _lidentifier_,
        @SuppressWarnings("hiding") TDot _dot_)
    {
        // Constructor
        setLidentifier(_lidentifier_);

        setDot(_dot_);

    }

    @Override
    public Object clone()
    {
        return new AIdentifierPrefix(
            cloneNode(this._lidentifier_),
            cloneNode(this._dot_));
    }

    @Override
    public void apply(Switch sw)
    {
        ((Analysis) sw).caseAIdentifierPrefix(this);
    }

    public TLidentifier getLidentifier()
    {
        return this._lidentifier_;
    }

    public void setLidentifier(TLidentifier node)
    {
        if(this._lidentifier_ != null)
        {
            this._lidentifier_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._lidentifier_ = node;
    }

    public TDot getDot()
    {
        return this._dot_;
    }

    public void setDot(TDot node)
    {
        if(this._dot_ != null)
        {
            this._dot_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._dot_ = node;
    }

    @Override
    public String toString()
    {
        return ""
            + toString(this._lidentifier_)
            + toString(this._dot_);
    }

    @Override
    void removeChild(@SuppressWarnings("unused") Node child)
    {
        // Remove child
        if(this._lidentifier_ == child)
        {
            this._lidentifier_ = null;
            return;
        }

        if(this._dot_ == child)
        {
            this._dot_ = null;
            return;
        }

        throw new RuntimeException("Not a child.");
    }

    @Override
    void replaceChild(@SuppressWarnings("unused") Node oldChild, @SuppressWarnings("unused") Node newChild)
    {
        // Replace child
        if(this._lidentifier_ == oldChild)
        {
            setLidentifier((TLidentifier) newChild);
            return;
        }

        if(this._dot_ == oldChild)
        {
            setDot((TDot) newChild);
            return;
        }

        throw new RuntimeException("Not a child.");
    }
}
